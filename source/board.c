...
void BOARD_PlayTurn(BOARD_Board* board)
{
  ...
  printf("x1=");
  scanf("%d", &board->selectedX);
  printf("%d\n", board->selectedX);
  printf("y1=");
  scanf("%d", &board->selectedY);
  printf("%d\n", board->selectedY);
  printf("x2=");
  scanf("%d", &x);
  printf("%d\n", x);
  ...
  printf("%d\n", y);
  // velmi shady xd
  pos = (uint8_t)y*8+(uint8_t)x;

  printf("sprav pohyb na %d %d\n",board->selectedY*8+board->selectedX, pos);
  ...
  for(uint8_t i=0;i<board->board.legalMoves.count;i++){ 
    if(board->board.legalMoves.list[i].from==(board->selectedY*8+board->selectedX) && board->board.legalMoves.list[i].to==pos){
      ...
      // printf("%s Q%d K%d\n",isWhite?"white":"black", 
      //   UTIL_GetBoolFromBools(board->board.bools, (isWhite)?INDEX_CCWQ:INDEX_CCBQ), 
